...
DEFAULT_SPLIT = "test"
DEFAULT_PARQUET = Path("/home/tranduong/dev/dataset/Video-MME/videomme/test-00000-of-00001.parquet")
...
def load_official_videomme_rows(parquet_path: str | Path = DEFAULT_PARQUET) -> list[dict[str, Any]]:
    import pandas as pd

    path = Path(parquet_path)
    df = pd.read_parquet(path)
    flat_rows = [dict(row) for row in df.to_dict(orient="records")]
...
class VideoMMELoader:
    ...
    def _load_raw_videos(self) -> list[dict[str, Any]]:
        if self.manifest_path is None:
            return load_official_videomme_rows(self.parquet_path)
        ...
        for item in raw_videos:
            questions = [
                VideoMMEQuestion(
                    ...
                    duration=str(item["duration"]).lower(),
